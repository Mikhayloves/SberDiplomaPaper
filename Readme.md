...
Это класс, который используется для заполнения базы данных начальными данными (seed data) в режиме разработки (dev). 
...
Для знакомства с тестами репозитрного слоя : ссылка фото

Для знакомства с тестами сервисов : ссылка фото
